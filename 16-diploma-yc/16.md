# DEVOPS-14: Дипломный практикум по профессии "DevOps-инженер"
...
secret: YCMDDN-RmCLfNvEuY5HJ4PXMfa1w64q-cVXfEVFt
````
...
Создадим директорию с именем [helm](./helm), перейдём в неё и создадим в ней заготовку чартов нашего веб-приложения:
...
В папке [helm](./helm) появилась соответствующая директория [web-app](helm/web-app).
...
Если перейти по [сгенерированному Github URL](https://olezhuravlev.github.io/diploma-chart/), то мы увидим
...
> в противном случае образ не пройдет проверку безопасности артефактория Helm!
...
Теперь, когда наше веб-приложение добавлено в репозиторий мы можем устанавливать его в кластер каким-либо
автоматизированным способом еще на этапе сборки кластере.
...
Развернём с помощью Терраформа еще два экземляра виртуальных машин на основе образов, оптимизированных для запуска
Docker-контейнеров:
...
![yc_vm_5.png](images/yc_vm_5.png)
...
[сервер и агент Teamcity](./ansible/infrastructure/playbooks/deploy-tc.ansible.yaml).
...
#### Предварительная настройка Teamcity
...
![tc_db.png](./images/tc_db.png)

После скачивания актуального JDBC-драйвера, которое произойдет после нажатия соответствующей
кнопки, мы окажемся на странице лицензии, с которой согласимся не читая:

![tc_license.png](./images/tc_license.png)

Нам будет предложено задать пароль администратора. Используем необычное сочетание `admin/admin`:

![tc_admin.png](./images/tc_admin.png)

Мы оказываемся на странице приветствия:

![tc_welcome.png](./images/tc_welcome.png)

Создавать проект пока не будем, а перейдем на закладку Agents (вверху страницы) и увидим, что агент
присутствует, но не авторизован:

![tc_agent_unauth.png](./images/tc_agent_unauth.png)

Авторизуем агента (комментарий вводить не обязательно):

![tc_agent_auth.png](./images/tc_agent_auth.png)

После этого вернемся на страницу приветствия (нажав на закладку "Projects" в верхней части
страницы) и создадим новый проект, который назовем "Diploma Repo", указав ему адрес репозитория,
изменения в котором должны отслеживаться:

![tc_git_repo.png](./images/tc_git_repo.png)

Так как в корне нашего репозитория находится файл сборки образа Dockerfile, то Teamcity, обнаружив
его, предложит нам в качестве этапа сборки создание нашего Docker-образа. Так как это входит
в наши планы, то отметим этот этап галочкой и нажмём "Use selected":

![tc_docker_autodetected.png](./images/tc_docker_autodetected.png)
...
![tc_connection.png](./images/tc_connection.png)
...
![tc_build_features.png](./images/tc_build_features.png)
...
![tc_build_features_docker.png](./images/tc_build_features_docker.png)

После сохранения поддержка Docker для данной сборки подключена:

![tc_build_features_done.png](./images/tc_build_features_done.png)


#### Настройка процедуры сборки в Teamcity
...
> Следует обращать внимание, что агент Teamcity должен иметь доступ ко всем инструментам, которые ему требуются
...
Для этого на закладке "Build Step" выбирает тип шага "Command Line":

![tc_step_cmd.png](./images/tc_step_cmd.png)

Далее, заходим в настройки этого шага задаём скрипт, который должен исполняться:

![tc_step_1.png](images%2Ftc_step_1.png)
...
![tc_parameters.png](./images/tc_parameters.png)
...
Далее следует зайти в редактирование второго шага нашей сборки - создание Docker-образа - и указать
параметры создаваемого образа:

![tc_step_2.png](./images/tc_step_2.png)

Все необходимые поля будут заполнены автоматически, нам лишь нужно указать наименование
собираемого образа и его тег, значение которого будет извлекаться из заполненной на первом шаге
...
> пользователя Github (здесь - это `olezhuravlev`)!
> 
...
> выполнения шага:
>
> ![tc_step_2_add_condition.png](./images/tc_step_2_add_condition.png)
>
...
>
> ![tc_step_2_condition.png](./images/tc_step_2_condition.png)

Следующим создадим шаг по отправке собранного образа в репозиторий. Для этого в настройках сборки
(сборка -> кнопка "Edit configuration") на закладке "Build Steps" добавляем новый шаг для Docker:

![tc_step_docker.png](./images/tc_step_docker.png)
...
![tc_step_3.png](images%2Ftc_step_3.png)
...
- обновление номера версии приложения файлах [values.yaml](./helm/web-app/values.yaml) и [Chart.yaml](./helm/web-app/Chart.yaml);
...
![tc_step_4.png](images%2Ftc_step_4.png)
...
>
> ![github_token.png](./images/github_token.png)
>
...
>
>````bash
>$ git clone https://olezhuravlev:%GITHUB_PERSONAL_TOKEN%@github.com/olezhuravlev/%HELM_REPO_NAME%.git;
> ````

И последним, 5-м шагом будет модификация файлов Git-репозитория, связанного с Helm-артефакторием. Здесь нам нужно изменить
версии приложения в двух файлах -  `Chart.yaml` и `values.yaml`, после чего создать Helm-архив и индексный файл новой
версии. Все действия, как и на предыдущем шаге, выполняются утилитами командной строки:

![tc_step_5.png](images%2Ftc_step_5.png)
...
![tc_steps_1_5.png](images%2Ftc_steps_1_5.png)

Осталось выполнить последний этап процесса - обновление нашего работающего в кластер веб-приложения до последней версии.
Для этого существует множество способов, но мы воспользуемся
[CRON-планировщиком](/run/media/oleg/Second/Netology/devops-diploma-yandexcloud/16-diploma-yc/ansible/infrastructure/playbooks/deploy-web-app-cron.ansible.yaml),
запускающегося каждые 30 минут на контрольном узле кластера и выполняющего несложный скрипт обновления репозитория Helm
(иначе новая версия приложения не будет видна) и обновления приложения:
...
В виде CRON-расписание выполнение процедуры обновленя выглядит следующим образом:

````bash
ubuntu@cp1:/usr/local/bin$ crontab -l
#Ansible: helm-web-app-update-30min
*/30 * * * * /usr/local/bin/helm repo add diploma-chart https://olezhuravlev.github.io/diploma-chart/charts/; /usr/local/bin/helm repo update; /usr/local/bin/helm upgrade --install web-app diploma-chart/web-app --atomic;
````


#### Итоговая проверка процедуры сборки в Teamcity

Т.к. конечным этапом всего цикла сборки является обновление рабочей нагрузки (веб-приложения) в работающем кластере
Kubernetes, то отметим состояние веб-приложения до начала автоматизированной сборки. Ориентироваться будем на
версию приложения и здесь она `0.0.7`:

![webapp_007.png](images%2Fwebapp_007.png)

Обновим наше веб-приложение, присвоим коммиту версию `0.0.8` и отправим её в репозиторий:

````bash
commit c952e6818d15c507fee228d373314cedb3ed1833 (HEAD -> main, tag: 0.0.8, origin/main)
Author: Oleg Zhuravlev <olezhuravlev@gmail.com>
...
![dockerhub_008.png](images%2Fdockerhub_008.png)

> Если на этапе сборки происходит ошибка:
>
> ![dockerhub_denied.png](images/dockerhub_denied.png)
>
> То это значит, что не выполнен этап включения поддержки "Docker", выполняемый на закладке "Build Features"!

Также окажется обновленным Helm-архив в Git-репозитории, связанным с Helm-артифакторием:

![github_chart_008.png](images%2Fgithub_chart_008.png)

Спустя некоторое время (~30 минут) обновляется и Helm-артифакторий:

![artifactory_008.png](images%2Fartifactory_008.png)

Каждые 30 минут срабатывает расписание CRON, которое обновляет чарт и, соответственно, обновляется наше веб-приложение:

![webapp_008.png](images%2Fwebapp_008.png)

Таким образом, мы добились того, что при появлении в репозитории нового релиза приложения, выполняется полный цикл
развертывания приложения.

Кратко схему взаимодействия можно представить следующим образом:

![build_diagramm.jpg](images%2Fbuild_diagramm.jpg)
...
TEST SUITE: None
NOTES:
Welcome to "web-app" (A demo Helm chart for Kubernetes) version "0.0.6" for namespace "default",
for current namespace "",
proudly build from repository "olezhuravlev/diploma".
...
````bash
ubuntu@cp1:~$ crontab -l
#Ansible: test
*/2 * * * * helm repo update; helm upgrade --install web-app diploma-chart/web-app;
````
...
Удалить все задачи CRONTAB:
````bash
ubuntu@cp1:~$ crontab -r
ubuntu@cp1:~$ crontab -l
...
Ошибки CRON:
````bash
ubuntu@cp1:/var/mail$ ls -ls
total 8
4 -rw------- 1 root   mail 1756 Jan 25 14:28 root
4 -rw------- 1 ubuntu mail 3772 Jan 25 14:34 ubuntu
````

Логи CRON:
````bash
ubuntu@cp1:/usr/local/bin$ grep CRON /var/log/syslog
Jan 25 15:00:01 cp1 CRON[348047]: (ubuntu) CMD (/usr/local/bin/helm repo add diploma-chart https://olezhuravlev.github.io/diploma-chart/charts/; /usr/local/bin/helm repo update; /usr/local/bin/helm upgrade --install web-app diploma-chart/web-app --atomic;)
````
