...
обновить версию приложения в репозитории. Для этого в файлах [values.yaml]() [Chart.yaml]() изменяем номер версии на следующий и снова создаём архив приложения:
...
И обновляем индексный файл [index.yaml](./helm/charts/index.yaml):
````bash
$ helm repo index charts/
````

Создан новый архив и обновлен индесный файл.Теперь отправляем все изменения в репозиторий на Github:

![github_helm_repo2.png](./images/github_helm_repo2.png)

> Как видим, в репозитории оказалось уже две версии нашего приложения - удалять старые версии нельзя, потому что
> они уже могут быть в работе у пользователей репозитория.

Через некоторое время сервис [artifacthub.io](https://artifacthub.io/) посетит наш репозиторий и, обнаружив, что
файл [index.yaml](./helm/charts/index.yaml) обновлён, перечитает репозиторий, обнаружит в нём
[файл метаданных](./helm/charts/artifacthub-repo.yml) и присвоит нашему репозиторию
почётный статус "Verified Publisher":


Теперь, когда наше веб-приложение добавлено в репозиторий мы можем устанавливать его на этапе сборки 


````bash
key-1
API-KEY-ID: de7407ec-b6a4-4328-a30a-fcd35485a2e6
API-KEY-SECRET: L5qbhNGS4Rvw9DqYhoKeJ6lIF9RpELUXqcAINTDuWVo=
````
...
Qbec - шаблонизатор и средство деплоя в кластер. Это не package manager.
Может обеспечить доставку в разные кластеры.
Разные окружения (dev/stage).
Разный состав деплоев в разные кластеры.
Разные версии в разные кластеры.

qbec.yaml - параметры соединения с кластером.
params.libsonnet - сопоставления переменных окружения с параметрами, указанными в qbec.yaml

Инициализация Qbec:

````bash
$ qbec init monitoring --with-example
using server URL "https://158.160.37.208:6443" and default namespace "default" for the default environment
wrote monitoring/params.libsonnet
wrote monitoring/environments/base.libsonnet
wrote monitoring/environments/default.libsonnet
wrote monitoring/components/hello.jsonnet
wrote monitoring/qbec.yaml
````
