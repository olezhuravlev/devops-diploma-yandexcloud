...
- [Chart.yaml](./helm/web-app/Chart.yaml) - описание чарта, включая номер его версии;
- [values.yaml](./helm/web-app/values.yaml) - описание используемого чартом Docker-образа;
- [.helmignore](./helm/web-app/.helmignore) - список игнорируемых при сборке пакета файлов;
- [папка `templates`](./helm/web-app/templates), содержащая файлы:
  - [deploy-web-app.yaml](./helm/web-app/templates/deploy-web-app.yaml) - Kubernetes-манифесты, применяемые при разворачивании приложения в кластере;
  - [_helpers.tpl](./helm/web-app/templates/_helpers.tpl) - шаблоны, используемые чартом;
  - [NOTES.txt](./helm/web-app/templates/NOTES.txt) - сообщение, выводимое пользователю при применении чарта;
...
Полученный в результате файл [index.yaml](./helm/charts/index.yaml) содержит перечень предоставляемых
...
Создадим такой файл, назвав его [index.html](./helm/index.html):
````html
<html>
<head>
  <meta http-equiv="Content-Type" content="text/html;charset=UTF-8">
...
После того, как тег последнего коммита, отмечающий что данный коммит является релизным, получен, перейдем
к следующему шагу - сборка нового Docker-образа нашего приложения.
...
Последним, 5-м шагом будет модификация файлов Git-репозитория, связанного с Helm-артефакторием. Здесь нам нужно изменить
...
[CRON-планировщиком](./ansible/infrastructure/playbooks/deploy-web-app-cron.ansible.yaml),
...
*/30 * * * *
 /usr/local/bin/helm repo add diploma-chart https://olezhuravlev.github.io/diploma-chart/charts/;
...
значительных усилий и затрат со стороны инженерного персонала.
...
---
_**Netology, октябрь 2021 - январь 2023 гг.**_
