...
![tc_step_1_cmd_retrieve_tag.png](./images/tc_step_1_cmd_retrieve_tag.png)
...
![tc_step_2_docker_build.png](./images/tc_step_2_docker_build.png)

>  Обращаем внимание, что имя образа включает в себя отделённое слешем имя используемого репозитория
> (здесь - это `olezhuravlev`)!
...
Создадим шаг по отправке собранного образа в репозиторий. Для этого возвращаемся в настройки сборки
(сборка -> кнопка "Edit configuration") и на закладке "Build Steps" добавляем новый шаг:
...
![tc_step_3_docker_push.png](./images/tc_step_3_docker_push.png)

> Т.к. мы хотим следовать **принципу семантического версионирования**, то дополнительно настроим требование к тегу,
> присвоенному коммиту и используемому на 2-м и 3-м шагах сборки. Для этого вернёмся к шагу №2 и добавим условие
> выполнения шага:
>
...
>
> После добавления условия шаг №2 будет выглядеть следующим образом: 
>
> ![tc_step_2_with_condition.png](./images/tc_step_2_with_condition.png)

Настройка в целом закончена, и в итоге состав наших шагов будет выглядет следующим образом:

![tc_steps_1_3.png](./images/tc_steps_1_3.png)

Нажатием кнопки "Run" запустим сборку, которая успешно проходит:

![tc_build_done.png](images/tc_build_done.png)

После успешного прохождения сборки собранный образ оказывается в нашем репозитории Dockerhub:

![dockerhub_004.png](images/dockerhub_004.png)
...
#### Настройка процедуры обновления версии в кластере Kubernetes

Веб-приложение попадает в кластер посредством Kubernetes-репозитория Helm, размещенного отдельно от репозитория
с веб-приложением и привязанного к Helm-артефакторию. Корректное обновление артефактория состоит из следующих
этапов:
...
Реализуем работу данной схемы, используя Teamcity.

| Шаг | Действие                                                                                        | Используемые команды                                                              |
|:---:|:------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------|
|  1  | Получение тега последнего коммита в репозитории приложения                                      | git tag --points-at HEAD                                                          |
...
> в процессе сборки, таким как Git или Helm!

В итоге шаги сборки выглядят следующим способом:

![tc_steps_1_5.png](./images/tc_steps_1_5.png)
...
![github_token.png](images%2Fgithub_token.png)
...
При этом имя github-репозитория указывается вместе с именем пользователя, т.к. нашем случае шаблон применяется
следующим образом:
...
Выражения, обрамлённые с помощью символа `%` - это параметры, заданные в Teamcity. По итогу используется следующий
набор параметров:

![tc_params.png](images%2Ftc_params.png)



````bash
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
````
