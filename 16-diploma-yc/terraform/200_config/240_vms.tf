...
module "vm-for-each" {
  ...
  for_each      = toset(["cp1", "node1", "node2", "tc-server", "tc-agent"])
...
locals {
  ...
  image_family = {
    ...
    tc-server = "ubuntu-2004-lts"
    tc-agent  = "container-optimized-image"
  }
  ...
  ipv4_zones = {
    ...
    tc-server = var.yandex-cloud-zone1
    tc-agent  = var.yandex-cloud-zone1
  }
  ...
  ipv4_subnets = {
    ...
    tc-server = yandex_vpc_subnet.vpc-subnet-private1.id
    tc-agent  = yandex_vpc_subnet.vpc-subnet-private1.id
  }
  ...
  ipv4_internals = {
    ...
    node1     = "10.20.20.1"
    node2     = "10.30.30.1"
    tc-server = "10.10.10.10"
    tc-agent  = "10.10.10.20"
  }
